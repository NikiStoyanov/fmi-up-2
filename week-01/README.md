# Увод в програмирането със C++. Среда за разработка (IDE). Примитивни типове данни. Основни аритметични операции. Вход и изход. Условни оператори.
...
### Lazy Evaluation при `&&` и `||`
В `C++` при тези оператори изчисляването винаги се извършва строго отляво надясно и спира в секундата, в която крайният резултат стане известен:

При логическо И (`&&`): Ако левият операнд се оцени като `false`, десният изобщо не се изпълнява, тъй като общият резултат вече гарантирано е `false` (независимо от дясната страна).

При логическо ИЛИ (`||`): Ако левият операнд се оцени като `true`, десният изобщо не се изпълнява, защото общият резултат вече със сигурност е `true`.
...
int main() {
    int x = 0;
    bool isValid = false;
    // Ако isValid е true, x става 1. Иначе x става 2.
    (isValid && (x = 1)) || (x = 2);
    std::cout << "Резултат: " << x << '\n'; 
}
```
...
<img src="assets/priority_and_associativity_1.webp" alt="img" width="600" />
<img src="assets/priority_and_associativity_2.webp" alt="img" width="600" />
